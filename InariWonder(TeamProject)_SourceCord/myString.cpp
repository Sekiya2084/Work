...
int MyString::m_HiraSize = -1;
int MyString::m_KataSize = -1;
int MyString::m_KigoSize = -1;
int MyString::m_EngSize = -1;


MyString::MyString(string text)
{
	//テクスチャが読み込まれてなかったら
	//if(m_StringTex.texNo == NULL){
		m_StringTex = new TEXTURE_DATA((char*)"data/TEXTURE/hira.png", (UINT8)16, (UINT8)16, (UINT8)255);
	//}

	//これいらんな
	if (m_HiraSize == -1) {
		m_HiraSize = sizeof(m_Hira) / sizeof(char);
	}
	if (m_HiraSize == -1) {
		m_HiraSize = sizeof(m_Kata) / sizeof(char);
	}
	if (m_HiraSize == -1) {
		m_HiraSize = sizeof(m_Kigo) / sizeof(char);
	}
	if (m_HiraSize == -1) {
		m_HiraSize = sizeof(m_Eng) / sizeof(char);
	}
...
void MyString::DrawString(D3DXVECTOR2 pos, D3DXVECTOR2 size,D3DXCOLOR col)
{
	...
	for (int i = 0; i < m_Size; i++) {
		pp.x = pos.x + size.x * i;
		DrawAnimation_LT(*m_StringTex, pp, size, col, (int)m_Text[i]);
	}
}
